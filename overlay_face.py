...
from generate import TalkingFacePredictor, extract_audio_features, extract_audio_features_from_video

def overlay_landmarks(image, shape):
    overlayed_image = np.copy(image)
    try:
        for part in shape.parts():
            overlayed_image[part.y-2:part.y+2, part.x-2:part.x+2] = np.array([255, 255, 255])
...
def process_video(video_path:str, output_dir:str, model:TalkingFacePredictor=None, save_images:bool=False):
    ...
    for frame_idx in tqdm(range(video.count_frames())):
        try:
            ...
            dets = detector(image, 0)  # dlib type `rectangles`, indexable. Each rectangle is ordered left(), top(), right(), bottom(), so x0, y0, x1, y1
            if len(dets) != 1:
                return f"FOUND {len(dets)} FACES! (only want 1)"
            shape = predictor(image, dets[0])  # shape.parts() has 68 points, each which has (x, y) attributes, all uint8 giving absolute position in image
            ...
            if save_images:
                with open(Path(output_dir) / f"{video_path.replace('/', '-')}_frame-{frame_idx:03d}.png", 'wb') as f:
...
def parse_args():
    ...
    parser.add_argument('--input-video', help='Video on which to overlay face landmarks')
    ...
    parser.add_argument('--save-images', required=False, action='store_true', help='Saves individual video frames as images')
...
if __name__ == '__main__':
    args = parse_args()
    process_video(video_path=args.input_video, output_dir=args.output_dir, model=args.model, save_images=args.save_images)
