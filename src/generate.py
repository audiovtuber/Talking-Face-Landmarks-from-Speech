    ...
    def predict(self, inputs: np.ndarray):
        with torch.no_grad():
            if self.hidden_state is None:
                pred, (self.hidden_state, self.cell) = self.model(
                    torch.tensor(inputs).float()
                )
            else:
                pred, (self.hidden_state, self.cell) = self.model(
                    torch.tensor(inputs).float(), self.hidden_state, self.cell
                )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="Delay in terms of number of frames, where each frame is 40 ms",
    )
...
if __name__ == "__main__":
    ...
    utils.write_video_wpts_wsound(
        out, sound, 44100, args.out_fold, "PD_pts", [0, 1], [0, 1]
    )
