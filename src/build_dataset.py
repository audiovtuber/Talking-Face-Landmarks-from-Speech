...
def extract_features(path: str, overwrite: bool = False) -> str:
    ...
    if (
        os.path.exists(f"{output_dir}/{video_id}-frames.npy")
    ...
    # Mean face shape, you can use any kind of face instead of mean face.
    ms = np.load("mean_shape.npy")
    ...
    # shape after transpose is roughly (audio_length_in_ms / 40, 64)
    melFrames = np.transpose(utils.melSpectra(y, sr, wsize, hsize))

    # inserts zeros into the first row of the diff of melFrames (the diff operation wraps and we don't want that)
    melDelta = np.insert(np.diff(melFrames, n=1, axis=0), 0, zeroVecD, axis=0)
    # same idea as last line. n=2 is equal to np.diff(np.diff(x, n=1), n=1), so the first two rows should be zeroes
...
if __name__ == "__main__":
    ...
    if num_processes > 1:
        ...
        results = []
        for result in tqdm(
            pool.imap_unordered(extract_features, video_paths), total=len(video_paths)
        ):
            results.append(result)
    else:
        print(
            "Only using one process. This takes forever and is only useful for debugging"
        )
